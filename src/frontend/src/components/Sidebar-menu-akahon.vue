    ...
    <div style="display: flex ; flex-direction:column; justify-content: space-between; flex-grow: 1; max-height: calc(100% - 60px); ">
      ...
      >
        ...
        >
          ...
          >
            <li>
              <a v-on:click="selectMenu(menuItem.link)">
                <i
                  class="bx"
      ...
      <div
        class="profile"  
      >
        <div class="profile-details" v-on:click="selectMenu('user')">
          <img
            v-if="profileImg"
            :src="profileImg"
            alt="profileImg"
      ...
      selectMenu (info) {
        if (info == "user") {
          if (!this.$store.state.markers.logged) {
            this.$parent.loginButton("Login")

          }else{
            this.$parent.loginButton("Profile")
          }
        }else{
          if (info == "logout"){
          ...
          if (this.$parent.markers.allowSw.includes(info)){
            this.$parent.markers.pageStat = info
          }else{
            this.$parent.loginButton(info)
          }
        }
      }
    },
    computed: {
      cssVars() {
        return {
          ...
          '--search-color': this.searchPlaceholder,
        }
      },
    },
    watch: {
